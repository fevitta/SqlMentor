...
- [x] 11. Escrever testes para R4, R6 e orquestrador
  - [x] 11.1 Testes para `_collapse_orphan_predicates_by_ids`
  ...
  - [x] 11.2 Escrever property test — Property 5: Consistência de predicados
  ...
  - [x] 11.3 Testes para `_add_nonsequential_id_note`
    - IDs sequenciais (1,2,3): sem nota inserida
    - IDs com salto (1,3,5): nota inserida após `Plan hash value`
    - _Requirements: 8.1, 8.2_

  - [x] 11.4 Escrever property test — Property 12: Nota de IDs não sequenciais
  ...
  - [x] 11.5 Testes para `_compress_plan`
  ...
  - [x] 11.6 Escrever property test — Property 10: IDs colapsados ausentes do plano reconstruído
    - **Property 10: IDs colapsados ausentes do plano reconstruído**
    - **Validates: Requirements 9.3, 9.5**

  - [x] 11.7 Escrever property test — Property 11: Robustez de _compress_plan
...
- [x] 12. Escrever testes para to_markdown e integração CLI/MCP
  - [x] 12.1 Testes para `to_markdown` com verbosity
  ...
  - [x] 12.2 Escrever property test — Property 6: Verbosity inválido levanta erro
  ...
  - [x] 12.3 Escrever property test — Property 2: Monotonicidade de compressão
  ...
  - [x] 12.4 Escrever property test — Property 1: Idempotência de full
    - **Property 1: Idempotência de full**
    - **Validates: Requirements 1.2, 9.2**
    - Estratégia: `to_markdown(ctx, "full")` chamado duas vezes produz output idêntico

- [x] 13. Teste de integração com fixture de plano real
  - _Requirements: 1.2, 1.3, 10.1, 10.2_
  - [x] 13.1 Criar fixture `tests/fixtures/sample_plan.txt`
    - Copiar ou criar um plano de execução real (anonimizado, sem dados de produção) para uso nos testes de integração
  - [x] 13.2 Verificar que `compact` produz menos linhas que `full`
    - Usar a fixture criada em 13.1 e assert `len(compact_lines) < len(full_lines)`
    - _Requirements: 1.3_
  - [x] 13.3 Verificar regressão de `full` contra baseline
    - Gerar baseline do output `full` e verificar que chamadas subsequentes produzem output idêntico
    - _Requirements: 1.2, 10.1, 10.2_

- [x] 14. Checkpoint final — Garantir que todos os testes passam
  - Garantir que todos os testes passam, perguntar ao usuário se houver dúvidas.

- [x] 15. Atualizar documentação
  - [x] 15.1 Atualizar `powers/sqlmentor/POWER.md` com parâmetro `verbosity` nas tools `analyze_sql` e `inspect_sql`
    - Documentar os três valores (`full`, `compact`, `minimal`) e seus comportamentos
    - _Requirements: 10.2_

  - [x] 15.2 Atualizar `README.md` com exemplos de uso de `--verbosity`
