...
| Campo | Valor |
|---|---|
| **Descrição** | Criar ABCs `DatabaseAdapter`, `QueryBuilder` e `PlanParser` em módulo novo `src/sqlmentor/adapters/base.py` |
| **Depende de** | — (tarefa raiz) |
| **Bloqueia** | T2, T3, T5, T8, T10, T13, T17, T20 |
| **Esforço** | 3 dias |

**Escopo:**
- `DatabaseAdapter`: connect, disconnect, test_connection, validate_privileges, execute_query
- `QueryBuilder`: table_stats, column_stats, indexes, constraints, table_ddl, explain_plan, runtime_stats, etc. (30+ métodos, um por query atual)
- `PlanParser`: parse(lines) → list[PlanBlock]
- Factory function: `get_adapter(type: str) → DatabaseAdapter`
...
| Campo | Valor |
|---|---|
| **Descrição** | Extrair lógica Oracle de `connector.py` para `adapters/oracle.py`, manter `connector.py` como camada de roteamento |
| **Depende de** | T1, T8 |
| **Bloqueia** | T3, T4, T10, T17 |
| **Esforço** | 4 dias |

**Escopo:**
- Mover `oracledb` import, thick mode, DSN, `DPY-3010` handling para `OracleAdapter`
- `connector.py` passa a instanciar adapter via factory baseado no campo `type` do profile
- `test_connection()` e `diagnose_connection()` delegam para o adapter
...
| Campo | Valor |
|---|---|
| **Descrição** | Mover todas as queries de `queries/__init__.py` para `queries/oracle.py`, expondo via `OracleQueryBuilder` |
| **Depende de** | T1, T2 |
| **Bloqueia** | T4, T11, T18 |
| **Esforço** | 3 dias |

**Escopo:**
- `queries/__init__.py` vira re-export / factory
- `queries/oracle.py` implementa `QueryBuilder` com todas as 30+ queries Oracle atuais
...
| Campo | Valor |
|---|---|
| **Descrição** | `collect_context()` recebe adapter em vez de conexão crua; delega execução de queries e LOB handling ao adapter |
| **Depende de** | T2, T3 |
| **Bloqueia** | T5, T6, T7, T12, T19 |
| **Esforço** | 5 dias |

**Escopo:**
- Remover `import oracledb` do collector
- LOB `.read()` encapsulado no `OracleAdapter.execute_query()`
- `ALTER SESSION`, `SYS_CONTEXT`, `EXPLAIN PLAN FOR` delegados ao adapter
- `_collect_explain_plan()` e `_collect_runtime_execution()` chamam adapter
...
| Campo | Valor |
|---|---|
| **Descrição** | Extrair regexes de parsing DBMS_XPLAN para `OraclePlanParser`; `_compress_plan()` recebe `list[PlanBlock]` já parseados |
| **Depende de** | T1, T4 |
| **Bloqueia** | T7, T13, T15, T20, T22 |
| **Esforço** | 3 dias |

**Escopo:**
- `_PLAN_ROW` e `_PLAN_ROW_ESTIMATED` regexes → `OraclePlanParser`
- `_detect_plan_blocks()` delega para parser do adapter
- Regras R1-R12 continuam operando sobre `list[PlanBlock]` (sem mudança)
- `to_markdown()` recebe dialect para labels contextuais (ex: "V$SQL" vs "pg_stat_statements")
...
| Campo | Valor |
|---|---|
| **Descrição** | CLI e MCP detectam tipo de banco do profile e passam adapter correto; ajustar help texts e conceito de `sql_id` |
| **Depende de** | T4 |
| **Bloqueia** | T7, T16, T23 |
| **Esforço** | 2 dias |
...
- `inspect` aceita `--sql-id` (Oracle) ou `--query-id` (PG) ou `--digest` (MariaDB) — ou parâmetro genérico `--statement-id`
- `doctor` verifica dependências do adapter ativo (ex: `psycopg2` instalado?)
...
| Campo | Valor |
|---|---|
| **Descrição** | Garantir que toda a suite de testes existente continua passando após refatoração |
| **Depende de** | T4, T5, T6 |
| **Bloqueia** | Fase 2, Fase 3 |
| **Esforço** | 3 dias |

**Escopo:**
- Rodar `pytest` completo — cobertura ≥ 90% mantida
- Testes de integração Oracle (Docker) passando
- Fixtures de plano Oracle (`sample_plan.txt`) validadas com `OraclePlanParser`
...
| Campo | Valor |
|---|---|
| **Descrição** | Adicionar campo `type: oracle|postgresql|mariadb` ao schema de `connections.yaml` e ao fluxo `config add` |
| **Depende de** | T1 |
| **Bloqueia** | T2 |
| **Esforço** | 1 dia |

**Escopo:**
- Default `type: oracle` para backward compatibility
- `config add` pergunta tipo de banco (ou infere pela porta: 1521=Oracle, 5432=PG, 3306=MariaDB)
- Validação no load do profile
...
| Campo | Valor |
|---|---|
| **Descrição** | Parametrizar system tables, funções built-in e sintaxe de binds por dialeto SQL |
| **Depende de** | T1 |
| **Bloqueia** | T12, T19 |
| **Esforço** | 2 dias |

**Escopo:**
- `_ORACLE_SYSTEM_TABLES` → dicionário por dialect
- `_BUILTIN_FUNCTIONS` → dicionário por dialect (NVL vs COALESCE vs IFNULL)
- `parse_sql()` aceita `dialect` param (sqlglot já suporta)
- Bind syntax: `:param` (Oracle), `%(name)s` / `$N` (PG), `?` (MariaDB)
